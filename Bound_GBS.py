    ...
    for i in range(len(data[1])-2):
        ...
        for j in range(len(i_feature_sort)-1):
            if GB_list[i_feature_sort[j]].label != GB_list[i_feature_sort[j+1]].label:    #在第i个特征上，相邻ball异质，则认为二者在该维度上处于类边界上
                if GB_list[j] not in bnd_gbs:
                    bnd_gbs.append(GB_list[j])
                if GB_list[j+1] not in bnd_gbs:
                    bnd_gbs.append(GB_list[j+1])
                #根据二者的位置特性，分别将GB_list[j]在i特征上取最大的样本作为边界样本；
                #反之，GB_list[j]在i特征上取最小的样本作为边界样本
                tmp_sample1 = bound_ball_sampling(GB_list[j],i,1)
                for example in tmp_sample1:
                    if example[-1] not in postsampling_index:
                        postsampling_index.append(example[-1])
                        postsampling.append(example[:-1])  #最后采样时，去掉样本索引
                tmp_sample2 = bound_ball_sampling(GB_list[j+1],i,0)
                for example in tmp_sample2:
                    if example[-1] not in postsampling_index:
                        postsampling_index.append(example[-1])
                        postsampling.append(example[:-1]) #最后采样时，去掉样本索引
    return bnd_gbs,postsampling

#边界上的球，根据其与类边界的方向，采该方向上球内最近的样本
#flag=1 表示feature维度的正向；flag=0，则表示负向
#若多个样本在该维度上均能取到最小值或者最大值，则都采
def bound_ball_sampling(ball,i,flag):
    if len(ball.data) == 1:
        return ball.data
    else:
