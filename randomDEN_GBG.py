...
LastEditTime: 2023-11-24 11:15:49
Description: 

保留了单一样本构成粒球的情况,此时粒球的半径为0

while |待划分样本|>=|待划分低密度样本点|
    对于待划分的样本集合，以类标签划分，按照样本量降序，对于每一类样本集合，随机选择一个点作为中心点
    对于每个随机的中心点, 执行以下步骤:
    1.执行中心点检查步骤: (下述,会保留具有局部一致性的中心)
        从每个类中心出发,若其最近邻与其异质,则考察其k近邻:
            (1) 若与其最近的样本是同类样本,则取最大的连续同质距离 tmp_dis
            (2) 若与其最近的样本是异类样本
                a.若x的k近邻全是异类样本,则将x作为噪声,并删除;continue,对于本类中心点,不再执行后续的粒球构造流程；
                b.若x的k近邻中仅最近样本是异类样本,则将最近样本作为噪声删除,并在剩下的待划分样本中取连续同质距离 tmp_dis;
                b.若x的k近邻不全是异类样本,则将x作为标签lable的低密度点,不再作为备选中心点;continue,对于本类中心点,不再执行后续的粒球构造流程;
        取连续同质距离 tmp_dis
    2.考察由中心出发的局部一致距离划定的ball与当前已构造的粒球的overlap情况
        计算中心到前序粒球球面的距离,将最近距离作为最小距离作为冲突距离conf_dis;
    if tmp_dis<=conf_dis:
        将所有符合条件的同类样本作为成员构造粒球(几何中心,半径,成员);
    else:
        取满足距离<=conf_dis的所有同类样本作为成员构造粒球(几何中心,半径,成员).

更新待划分样本集,低密度集合,粒球集合.
             
对于所有待划分低密度点构造半径为0的粒球.

[1]Vuttipittayamongkol P, Elyan E. Neighbourhood-based undersampling approach for handling imbalanced and overlapped data[J]. Information Sciences, 2020, 509: 47-70.
'''
